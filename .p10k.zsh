# Generated by Powerlevel10k configuration wizard on 2020-02-09 at 04:29 UTC.
...
() {
  ...
  typeset -g POWERLEVEL9K_TIME_FORMAT='%D{%a %b %d}'
  ...
  function prompt_my_ntl_holiday() {
    local month=`date +%b`
    local day=`date +%d`
    day=$((10#$day))
    local todays_holiday="$(head -n 1 /home/harrison/myprograms/ntl-cal/${month}/${day})"
    if [[ "${todays_holiday:0:3}" = "Ntl" ]]; then
      p10k segment -f blue -t "${todays_holiday}"
    elif [[ "${todays_holiday:0:5}" = "World" || "${todays_holiday:0:8}" = "Interntl" ]]; then
      p10k segment -f magenta -t "${todays_holiday}"
    else
      p10k segment -f blue -t "${todays_holiday}"
    fi
  }
